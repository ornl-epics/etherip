        ...
        for (int i=0; i<tags.size(); ++i)
        {
            ...
            synchronized (tag)
            {
                ...
                case READING:
                    ...
                    EtherNetIP.logger.log(Level.FINE, "Read {0}", tag);
                    break;
                case WRITING:
                    // Finished writing, return to reading
                    tag.setState(Tag.State.READING);
                    EtherNetIP.logger.log(Level.FINE, "Wrote {0}", tag);
                    break;
                case TO_BE_WRITTEN:
                    // Received yet another value to be written while writing the
                    // previous one, so keep the TO_BE_WRITTEN state
                    EtherNetIP.logger.log(Level.FINE, "Wrote {0}, need to write updated value", tag);
                }
            }
        }
    }
