 ...
 * <p>
 * Network connection as well as buffer and session info that's used for the
...
public class TcpConnection extends Connection 
{
	...
	public TcpConnection(final String address, final int slot) throws Exception
	{
		...
		this.channel.connect(new InetSocketAddress(address, this.port)).get(this.timeout_ms, MILLISECONDS);
	}
	...
	public void write(final ProtocolEncoder encoder) throws Exception
	{
		...
		if (logger.isLoggable(Level.FINEST))
		{
			logger.log(Level.FINEST, "Data sent ({0} bytes):\n{1}", new Object[]
				{this.buffer.remaining(), Hexdump.toHexdump(this.buffer)});
		}

		int to_write = this.buffer.limit();
		while (to_write > 0)
		{
			final int written = ((AsynchronousSocketChannel) this.channel).write(this.buffer).get(this.timeout_ms, MILLISECONDS);
			to_write -= written;
			if (to_write > 0)
			{
				this.buffer.compact();
	...
	public void read(final ProtocolDecoder decoder) throws Exception
	{
		...
		do
		{
		    ((AsynchronousSocketChannel) this.channel).read(this.buffer).get(this.timeout_ms, MILLISECONDS);
		}
